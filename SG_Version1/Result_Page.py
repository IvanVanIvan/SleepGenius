...
class ResultPage:
    ...
    def make_suggestions(self):
        ...
        sleep_tips = [
            ...
            (3, 'Recommended room temperature is 16 to 20 degrees Celsius when going to sleep'),
    ...
    # function to show random suggek
    ...
    def SaveInDB(self):
        ...
        check_query = 'SELECT User_Name FROM UsersSleepData WHERE User_Name = ?'
        cursor.execute(check_query, (username,))
        existing_user = cursor.fetchone()


        if existing_user:
            # Check if any of the Hours_Slept columns already exists
            ...
            if hours_data is None:
                # The user does not exist in the database, insert the data in Hours_Slept1 column
                insert_query = 'INSERT INTO UsersSleepData (User_Name, Hours_Slept1) VALUES (?, ?)'
                insert_data = (username, hours_slept)
                cursor.execute(insert_query, insert_data)
                conn.commit()
            
            else:
                # The user exists, update the first empty Hours_Slept column
                if hours_data[0] is None:
                    update_query = 'UPDATE UsersSleepData SET Hours_Slept1 = ? WHERE User_Name = ?'
                    update_data = (hours_slept, username)
                elif hours_data[1] is None:
                    update_query = 'UPDATE UsersSleepData SET Hours_Slept2 = ? WHERE User_Name = ?'
                    update_data = (hours_slept, username)
                elif hours_data[2] is None:
                    update_query = 'UPDATE UsersSleepData SET Hours_Slept3 = ? WHERE User_Name = ?'
                    update_data = (hours_slept, username)
                elif hours_data[3] is None:
                    update_query = 'UPDATE UsersSleepData SET Hours_Slept4 = ? WHERE User_Name = ?'
                    update_data = (hours_slept, username)
                elif hours_data[4] is None:
                    update_query = 'UPDATE UsersSleepData SET Hours_Slept5 = ? WHERE User_Name = ?'
                    update_data = (hours_slept, username)
                elif hours_data[5] is None:
                    update_query = 'UPDATE UsersSleepData SET Hours_Slept6 = ? WHERE User_Name = ?'
                    update_data = (hours_slept, username)
                elif hours_data[6] is None:
                    update_query = 'UPDATE UsersSleepData SET Hours_Slept7 = ? WHERE User_Name = ?'
                    update_data = (hours_slept, username)
                else:
                    # All Hours_Slept columns have data, do nothing
                    pass
                
                # determine whether the update_query variable is defined in the current local scope
                if 'update_query' in locals():
    ...
    def open_History_page(self):
        self.SaveInDB()
        # Open the SG_Version1\History_Results.py file using subprocess
        subprocess.Popen(["python", "SG_Version1\History_Results.py"])
