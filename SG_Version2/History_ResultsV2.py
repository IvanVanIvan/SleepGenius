...
class HistoryResults:
    def __init__(self):
        ...
        if row:
            ...
            # there are 7 columns for hours slept (from 1 to 7)
            for i in range(1, 8):
                # Check if the column is NULL
                if row[i] is None:
                    hours_label = "Hours slept: No Sleep Data"
                else:
                    hours_label = "Hours slept: " + str(row[i])

                # Create a label for the message
                self.sleep_label = tk.Label(self.HistoryScreen, text=hours_label, bg="black", fg="white", font=("Blinker", 8, "bold"))
                # Set the position of the label
                self.sleep_label.place(x=55, y=210 + i * 30)
        else:
    ...
    def input_another_result(self):
        ...
        self.clearHoursTXT()
        # Open the SG_Version2\SleepData_InputV2.py file using subprocess
        subprocess.Popen(["python", "SG_Version2\SleepData_InputV2.py"])
