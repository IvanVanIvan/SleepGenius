...
class ResultPage:
        ...
        if existing_user:
            ...
            if hours_data is not None:
                ...
                for i, hours in enumerate(hours_data):
                    if hours is None:
                        update_query = f'UPDATE UsersSleepData SET Hours_Slept{i + 1} = ? WHERE User_Name = ?'
    ...
    def open_History_page(self):
        self.SaveInDB()
        # Open the SG_Version2\History_ResultsV2.py file using subprocess
        subprocess.Popen(["python", "SG_Version2\History_ResultsV2.py"])
    ...
    def Logout(self):
        ...
        self.clearUNameTXT()
        # Open the SG_Version2\landingV2.py file using subprocess
        subprocess.Popen(["python", "SG_Version2\landingV2.py"])
        # Close the main screen
        self.Result_screen.destroy()
